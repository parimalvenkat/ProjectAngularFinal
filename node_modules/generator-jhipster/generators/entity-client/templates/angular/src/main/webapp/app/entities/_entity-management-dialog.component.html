...
<form name="editForm" role="form" novalidate (ngSubmit)="save()" #editForm="ngForm">
    ...
    <div class="modal-header" style="background-color: #e9ecef">
        <h4 class="modal-title colorblue" id="my<%= entityClass %>Label" jhiTranslate="<%= keyPrefix %>home.createOrEditLabel">Create or edit a <%= entityClassHumanized %></h4>
        <button type="button" class="close colorblue" data-dismiss="modal" aria-hidden="true"
                (click)="clear()">&times;</button>
    </div>
    <div class="modal-body">
        <<%= jhiPrefixDashed %>-alert-error></<%= jhiPrefixDashed %>-alert-error>
        <div class="form-group" [hidden]="!<%=entityInstance %>.id">
            <label for="id" style="font-weight: bold" jhiTranslate="global.field.id">ID</label>
        ...
        <div class="form-group">
            <label class="form-control-label" jhiTranslate="<%= translationKey %>" for="field_<%= fieldName %>" style="font-weight: bold"><%= fieldNameHumanized %></label>
