...
<div *ngIf="<%= entityInstance %>">
    <h2 style="font-weight: bold"><span jhiTranslate="<%= keyPrefix %>.detail.title"><%= entityClassHumanized %></span> {{<%= entityInstance %>.id}}</h2>
    <hr>
    ...
    <dl class="row-md jh-entity-details">
        ...
        <dd>
            ...
            <%_ if (relationshipType === 'many-to-many') { _%>
            <span *ngFor="let <%= relationshipFieldName %> of <%= entityInstance %>.<%= relationshipFieldNamePlural %>; let last = last">
                <a [routerLink]="['/<%= otherEntityStateName %>', <%= relationshipFieldName %>?.id ]">{{<%= relationshipFieldName %>.<%= otherEntityField %>}}</a>{{last ? '' : ', '}}
            </span>
            <%_ } else { _%>
            <%_ if (dto === 'no') { _%>
