    ...
    <div class="breadcrumb-container">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-style"  [class.active]="true">
                <a ><span jhiTranslate="<%= keyPrefix %>home.title"><%= entityClassPluralHumanized %></span></a>
            </li>
        </ol>
    </div>
    <div class="page-title-container">
        <div class="page-title-column1">
            <h4 class="page-title-column1-title index-font"><%= entityClassHumanized %></h4>
            <p class="page-title-helptext">Create or view <%= entityClassHumanized %></p>
        </div>
        <div class="page-title-column2" align="right">
            ...
            <button *jhiHasAnyAuthority="'ROLE_ADMIN'" type="button"  [routerLink]="['/', { outlets: { popup: ['<%= entityUrl %>-new'] } }]"
                    class="btn btn-primary addbtn float_right"><i class="fa fa-plus-circle" aria-hidden="true"></i> Create <%= entityClassHumanized %></button>
...
<div class="row" style="margin: 0 30px" *ngIf="<%=entityInstancePlural %>">
...
<table class="table width100pct" style="margin-bottom:0; border: 1px solid #e4e3e3;">
    <thead>
    ...
    <th class="colorblue"<% if (pagination !== 'no') { %> jhiSortBy="id"<% } %>><span jhiTranslate="global.field.id">ID</span><% if (pagination !== 'no') { %> <span class="fa fa-sort"></span><% } %></th>
    <%_ for (idx in fields) { _%>
    <th class="colorblue"<% if (pagination !== 'no') { %> jhiSortBy="<%= fields[idx].fieldName%>"<% } %>><span jhiTranslate="<%=keyPrefix + fields[idx].fieldName %>"><%= fields[idx].fieldNameHumanized %></span><% if (pagination !== 'no') { %> <span class="fa fa-sort"></span><% } %></th>
    <%_ } _%>
    ...
    <th class="colorblue"<% if (pagination !== 'no') { %> jhiSortBy="<%=relationships[idx].relationshipName + (fieldName)%>"<% } %>><span jhiTranslate="<%= keyPrefix + relationships[idx].relationshipName%>"><%= relationships[idx].relationshipNameHumanized %></span><% if (pagination !== 'no') { %> <span class="fa fa-sort"></span><% } %></th>
    <%_ } _%>
    <%_ } _%>
    <th></th>
    </tr>
