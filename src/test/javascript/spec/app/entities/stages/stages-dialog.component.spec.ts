/* tslint:disable max-line-length */
import { ComponentFixture, TestBed, async, inject, fakeAsync, tick } from '@angular/core/testing';
import { NgbActiveModal } from '@ng-bootstrap/ng-bootstrap';
import { Observable } from 'rxjs/Rx';
import { JhiEventManager } from 'ng-jhipster';

import { Try1TestModule } from '../../../test.module';
import { StagesDialogComponent } from '../../../../../../main/webapp/app/entities/stages/stages-dialog.component';
import { StagesService } from '../../../../../../main/webapp/app/entities/stages/stages.service';
import { Stages } from '../../../../../../main/webapp/app/entities/stages/stages.model';
import { TenantDetailsService } from '../../../../../../main/webapp/app/entities/tenant-details';

describe('Component Tests', () => {

    describe('Stages Management Dialog Component', () => {
        let comp: StagesDialogComponent;
        let fixture: ComponentFixture<StagesDialogComponent>;
        let service: StagesService;
        let mockEventManager: any;
        let mockActiveModal: any;

        beforeEach(async(() => {
            TestBed.configureTestingModule({
                imports: [Try1TestModule],
                declarations: [StagesDialogComponent],
                providers: [
                    TenantDetailsService,
                    StagesService
                ]
            })
            .overrideTemplate(StagesDialogComponent, '')
            .compileComponents();
        }));

        beforeEach(() => {
            fixture = TestBed.createComponent(StagesDialogComponent);
            comp = fixture.componentInstance;
            service = fixture.debugElement.injector.get(StagesService);
            mockEventManager = fixture.debugElement.injector.get(JhiEventManager);
            mockActiveModal = fixture.debugElement.injector.get(NgbActiveModal);
        });

        describe('save', () => {
            it('Should call update service on save for existing entity',
                inject([],
                    fakeAsync(() => {
                        // GIVEN
                        const entity = new Stages(123);
                        spyOn(service, 'update').and.returnValue(Observable.of(entity));
                        comp.stages = entity;
                        // WHEN
                        comp.save();
                        tick(); // simulate async

                        // THEN
                        expect(service.update).toHaveBeenCalledWith(entity);
                        expect(comp.isSaving).toEqual(false);
                        expect(mockEventManager.broadcastSpy).toHaveBeenCalledWith({ name: 'stagesListModification', content: 'OK'});
                        expect(mockActiveModal.dismissSpy).toHaveBeenCalled();
                    })
                )
            );

            it('Should call create service on save for new entity',
                inject([],
                    fakeAsync(() => {
                        // GIVEN
                        const entity = new Stages();
                        spyOn(service, 'create').and.returnValue(Observable.of(entity));
                        comp.stages = entity;
                        // WHEN
                        comp.save();
                        tick(); // simulate async

                        // THEN
                        expect(service.create).toHaveBeenCalledWith(entity);
                        expect(comp.isSaving).toEqual(false);
                        expect(mockEventManager.broadcastSpy).toHaveBeenCalledWith({ name: 'stagesListModification', content: 'OK'});
                        expect(mockActiveModal.dismissSpy).toHaveBeenCalled();
                    })
                )
            );
        });
    });

});
